...
function renderActions() {
  ...
  <div class='button-row'><button id='newFamily'>Create Family</button><button id='saveBtn'>Save Now</button></div>`;
...
function createCharacter(data) { if (!state.activeFamilyId) return; const id = `c${Date.now()}`; const c = { id, ...data, familyId: state.activeFamilyId, maritalStatus: "unmarried", health: "Healthy", characterReputation: 0, relationships: [] }; state.characters.push(c); state.charactersById[id] = c; state.feed.unshift({ text: `${c.firstName} joined ${state.familiesById[state.activeFamilyId].name}.`, type: "player", time: "now" }); render(); saveGame(); }
